function extractGammaSARL2SARData(inRawFilename,inRawParFilename,outRawCpxFilename,outPROCparFilename,outSARparFilename,rg_3dB_bw,az_3dB_bw, polarization) % ,self_zero,rg_bw_frac,rg_kaiser_par,az_bw_frac,az_kaiser_par)

% extractGammaSARL2SARData extracts the given channnel of the real-valued 
% GammaSAR-L (GS-L) raw data and then
...
%
% Usage:
%    extractGammaSARL2SARData(inRawFilename,inRawParFilename,outRawCpxFilename,outPROCparFilename,outSARparFilename,rg_3dB_bw,az_3dB_bw, polarization,self_zero,rg_bw_frac,rg_kaiser_par,az_bw_frac,az_kaiser_par)
%
%
%
%    where:
%       inRawFilename       : GAMMA SAR (GS_L) raw, real-valued beat-signal (*.raw)
...
%
%
% Created:         2018-07-04 by Othmar Frey, adapted from extractGPRI2SARDataDerampFFT 1;
% Modified:        2018-08-14 by Othmar Frey, frey@gamma-rs.ch
% Modified:        2018-08-16 by Othmar Frey, frey@gamma-rs.ch
%                                           Corrected sign error in rps and
%                                           range distances due to neg.
%                                           chirp rate with downchirp
%
% Modified:        2019-02-19 by Othmar Frey, frey@gamma-rs.ch
%                               Added parameters for 
%                               1. window type and
%                               2. window parameter
%                               parameter as well as
%
% Modified:        2019-05-20 by Othmar Frey, frey@gamma-rs.ch
%                               Added azimuth windowing function to
%                               get rid of the signal artifacts
%
% Modified:        2019-09-06 by Othmar Frey, frey@gamma-rs.ch
...
rangeDim = rawParStruct.CGEN_num_samp;
block_length = rawParStruct.CGEN_num_samp+1;

SamplingRate = rawParStruct.ADC_sample_rate/2;   % sampling rate of complex-valued beat signal (= deramped signal) 
NrOfRgPix    =  floor(rawParStruct.CGEN_num_samp/2);     % number of samples of complex-valued beat signal (= deramped signal) 
...
%scale = ((abs(slr)./slr(round(rawParStruct.CGEN_num_samp/8))).^1.5)';
...
ts = rawParStruct.RAW_start_time;
strucPROCPar.title = ts;
strucPROCPar.sensor = 'Gamma SAR GS-L';

fprintf('\nraw data start time: %s\n',rawParStruct.RAW_start_time);

cell_time_start = strsplit(rawParStruct.RAW_start_time, ' ');   % split the line on whitespace
...
chirp_duration = rawParStruct.CGEN_num_samp/rawParStruct.ADC_sample_rate;

strucSARPar.title = ts;
strucSARPar.sensor_name = 'Gamma SAR GS-L';
strucSARPar.chirp_direction = 'DOWN_CHIRP';
...
strucSARPar.samples_per_record = rawParStruct.CGEN_num_samp;
